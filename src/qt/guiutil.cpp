...
#include <qt/superaxecoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SuperAxeCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SuperAxeCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new SuperAxeCoinAddressCheckValidator(parent));
}
...
bool parseSuperAxeCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no superaxecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("superaxecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!SuperAxeCoinUnits::parse(SuperAxeCoinUnit::AXE, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseSuperAxeCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseSuperAxeCoinURI(uriInstance, out);
}

QString formatSuperAxeCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("superaxecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SuperAxeCoinUnits::format(SuperAxeCoinUnit::AXE, info.amount, false, SuperAxeCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSuperAxeCoinConf()
{
    ...
    /* Open superaxecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SuperAxeCoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SuperAxeCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("SuperAxeCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for SuperAxeCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "superaxecoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("superaxecoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a superaxecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=SuperAxeCoin\n";
        else
            optionFile << strprintf("Name=SuperAxeCoin (%s)\n", ChainTypeToString(chain));
