# PSBT Howto for SuperAxeCoin Core

Since SuperAxeCoin Core 0.17, an RPC interface exists for Partially Signed SuperAxeCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/superaxecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for SuperAxeCoin transactions that are not fully signed
...
#### Multisig with multiple SuperAxeCoin Core instances
...
SuperAxeCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `superaxecoin-cli` is used.
...
Later, when *V* AXE has been received on *Amulti*, and Bob and Carol want to
