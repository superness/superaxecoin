...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<SuperAxeCoinUnit>("SuperAxeCoinUnit");
#else
    qRegisterMetaType<SuperAxeCoinUnit>("SuperAxeCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. superaxecoin_de.qm (shortcut "de" needs to be defined in superaxecoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. superaxecoin_de_DE.qm (shortcut "de_DE" needs to be defined in superaxecoin.qrc)
...
static const char* qt_argv = "superaxecoin-qt";

SuperAxeCoinApplication::SuperAxeCoinApplication()
...
void SuperAxeCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the SuperAxeCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", SuperAxeCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void SuperAxeCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool SuperAxeCoinApplication::createOptionsModel(bool resetSettings)
{
...
void SuperAxeCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new SuperAxeCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &SuperAxeCoinGUI::quitRequested, this, &SuperAxeCoinApplication::requestShutdown);
...
void SuperAxeCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void SuperAxeCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool SuperAxeCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void SuperAxeCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &SuperAxeCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &SuperAxeCoinApplication::handleRunawayException);
    connect(this, &SuperAxeCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &SuperAxeCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void SuperAxeCoinApplication::parameterSetup()
{
...
void SuperAxeCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void SuperAxeCoinApplication::requestInitialize()
{
...
void SuperAxeCoinApplication::requestShutdown()
{
...
void SuperAxeCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // superaxecoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SuperAxeCoinGUI::handlePaymentRequest);
            connect(window, &SuperAxeCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void SuperAxeCoinApplication::handleRunawayException(const QString &message)
{
...
void SuperAxeCoinApplication::handleNonFatalException(const QString& message)
{
...
WId SuperAxeCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool SuperAxeCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SuperAxeCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(superaxecoin);
    Q_INIT_RESOURCE(superaxecoin_locale);
    ...
    /// 6-7. Parse superaxecoin.conf, determine network, switch to network specific
    ...
    // superaxecoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
