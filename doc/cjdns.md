# CJDNS support in SuperAxeCoin Core

It is possible to run SuperAxeCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of SuperAxeCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the SuperAxeCoin network and individual nodes.
...
before it will work with your SuperAxeCoin Core node. This is described in steps
...
## Run SuperAxeCoin Core with CJDNS

Once you are connected to the CJDNS network, the following SuperAxeCoin Core
...
When enabled, this option tells SuperAxeCoin Core that it is running in an
...
IPv6 local network. This helps SuperAxeCoin Core perform better address management:
...
CJDNS support was added to SuperAxeCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `superaxecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/superaxecoin/superaxecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in SuperAxeCoin Core

There are several ways to see your CJDNS address in SuperAxeCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `superaxecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `superaxecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `superaxecoin-cli help getnodeaddresses` for details.
