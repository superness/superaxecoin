...
/** Call node.updateRwSetting() with SuperAxeCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because SuperAxeCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/superaxecoin/superaxecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because superaxecoin 22.x will not try to read these.
...
//! Convert enabled/size values to superaxecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from superaxecoin -prune setting.
...
//! Get pruning size value to show in GUI from superaxecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaySuperAxeCoinUnit")) {
        settings.setValue("DisplaySuperAxeCoinUnit", QVariant::fromValue(SuperAxeCoinUnit::AXE));
    }
    QVariant unit = settings.value("DisplaySuperAxeCoinUnit");
    if (unit.canConvert<SuperAxeCoinUnit>()) {
        m_display_superaxecoin_unit = unit.value<SuperAxeCoinUnit>();
    } else {
        m_display_superaxecoin_unit = SuperAxeCoinUnit::AXE;
        settings.setValue("DisplaySuperAxeCoinUnit", QVariant::fromValue(m_display_superaxecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if superaxecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_superaxecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<SuperAxeCoinUnit>() == m_display_superaxecoin_unit) return;
    m_display_superaxecoin_unit = new_unit.value<SuperAxeCoinUnit>();
    QSettings settings;
    settings.setValue("DisplaySuperAxeCoinUnit", QVariant::fromValue(m_display_superaxecoin_unit));
    Q_EMIT displayUnitChanged(m_display_superaxecoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/superaxecoin/superaxecoin/pull/8273
    ...
    // (https://github.com/superaxecoin-core/gui/issues/567).
