...
const std::string SuperAxeCoinGUI::DEFAULT_UIPLATFORM =
...
SuperAxeCoinGUI::SuperAxeCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SuperAxeCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SuperAxeCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SuperAxeCoinGUI::showModalOverlay);
...
SuperAxeCoinGUI::~SuperAxeCoinGUI()
{
...
void SuperAxeCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SuperAxeCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and superaxecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SuperAxeCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SuperAxeCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SuperAxeCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SuperAxeCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SuperAxeCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed SuperAxeCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed SuperAxeCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a superaxecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible SuperAxeCoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SuperAxeCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SuperAxeCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SuperAxeCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SuperAxeCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SuperAxeCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SuperAxeCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SuperAxeCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SuperAxeCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SuperAxeCoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &SuperAxeCoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &SuperAxeCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SuperAxeCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SuperAxeCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SuperAxeCoinGUI::showDebugWindow);
}

void SuperAxeCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SuperAxeCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SuperAxeCoinGUI::setCurrentWalletBySelectorIndex);
...
void SuperAxeCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SuperAxeCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SuperAxeCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SuperAxeCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SuperAxeCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SuperAxeCoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void SuperAxeCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SuperAxeCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SuperAxeCoinGUI::removeWallet);
...
WalletController* SuperAxeCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void SuperAxeCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SuperAxeCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SuperAxeCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SuperAxeCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SuperAxeCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SuperAxeCoinGUI::incomingTransaction);
    connect(this, &SuperAxeCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void SuperAxeCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void SuperAxeCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SuperAxeCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SuperAxeCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SuperAxeCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SuperAxeCoinGUI::createTrayIcon()
{
...
void SuperAxeCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SuperAxeCoinGUI::toggleHidden);
...
void SuperAxeCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SuperAxeCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SuperAxeCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SuperAxeCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SuperAxeCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SuperAxeCoinGUI::openClicked()
{
...
void SuperAxeCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SuperAxeCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SuperAxeCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SuperAxeCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SuperAxeCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SuperAxeCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SuperAxeCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SuperAxeCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to SuperAxeCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SuperAxeCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SuperAxeCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SuperAxeCoinGUI::updateHeadersSyncProgressLabel()
{
...
void SuperAxeCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SuperAxeCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SuperAxeCoinGUI::quitRequested);
...
void SuperAxeCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SuperAxeCoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &SuperAxeCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SuperAxeCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SuperAxeCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SuperAxeCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SuperAxeCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void SuperAxeCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SuperAxeCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SuperAxeCoinGUI::incomingTransaction(const QString& date, SuperAxeCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SuperAxeCoinUnits::formatWithUnit(unit, amount, true));
...
void SuperAxeCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SuperAxeCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SuperAxeCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SuperAxeCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SuperAxeCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SuperAxeCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SuperAxeCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SuperAxeCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SuperAxeCoinGUI::updateWindowTitle()
{
...
void SuperAxeCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SuperAxeCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SuperAxeCoinGUI::detectShutdown()
{
...
void SuperAxeCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SuperAxeCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SuperAxeCoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SuperAxeCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SuperAxeCoinGUI::subscribeToCoreSignals()
{
...
void SuperAxeCoinGUI::unsubscribeFromCoreSignals()
{
...
bool SuperAxeCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SuperAxeCoinUnit> units = SuperAxeCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SuperAxeCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SuperAxeCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SuperAxeCoinUnit u : SuperAxeCoinUnits::availableUnits()) {
        menu->addAction(SuperAxeCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SuperAxeCoinUnit newUnits)
{
    setText(SuperAxeCoinUnits::longName(newUnits));
}
