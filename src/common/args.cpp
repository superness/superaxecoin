...
const char * const SUPERAXECOIN_CONF_FILENAME = "superaxecoin.conf";
const char * const SUPERAXECOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //superaxecoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", SUPERAXECOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\SuperAxeCoin
    // macOS: ~/Library/Application Support/SuperAxeCoin
    // Unix-like: ~/.superaxecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuperAxeCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/SuperAxeCoin";
#else
    // Unix-like
    return pathRet / ".superaxecoin";
#endif
#endif
}
