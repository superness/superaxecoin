...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing superaxecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='superaxecoin-cli', help='superaxecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='SuperAxeCoin address to which the faucet should send')
...
parser.add_argument('superaxecoin_cli_args', nargs='*', help='Arguments to pass on to superaxecoin-cli (default: -signet)')

args = parser.parse_args()

if args.superaxecoin_cli_args == []:
    args.superaxecoin_cli_args = ['-signet']


def superaxecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.superaxecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = superaxecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = superaxecoin_cli(['getnewaddress', 'faucet', 'bech32'])
