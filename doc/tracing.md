# User-space, Statically Defined Tracing (USDT) for SuperAxeCoin Core

SuperAxeCoin Core includes statically defined tracepoints to allow for more
...
**Note**: SuperAxeCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to SuperAxeCoin Core
...
Multiple tools can list the available tracepoints in a `superaxecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in SuperAxeCoin Core, use `info probes` in `gdb`:

```
$ gdb ./src/superaxecoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/superaxecoind
stap net        outbound_message 0x0000000000107c05 /src/superaxecoind
stap validation block_connected  0x00000000002fb10c /src/superaxecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in SuperAxeCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/superaxecoind | grep NT_STAPSDT -A 4 -B 2
