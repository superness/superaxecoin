...
""" Example logging SuperAxeCoin Core mempool events using the mempool:added,
...
def main(superaxecoind_path):
    superaxecoind_with_usdts = USDT(path=str(superaxecoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    superaxecoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    superaxecoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    superaxecoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    superaxecoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[superaxecoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/superaxecoind")
        exit(1)
