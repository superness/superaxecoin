# Bootstrappable SuperAxeCoin Core Builds

This directory contains the files necessary to perform bootstrappable SuperAxeCoin
Core builds.
...
To build SuperAxeCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      SuperAxeCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/superaxecoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/superaxecoin-core/guix.sigs).
