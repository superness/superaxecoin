...
class ConfArgsTest(SuperAxeCoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in superaxecoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "superaxecoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'superaxecoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "superaxecoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by superaxecoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a superaxecoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.superaxecoinconf.read_text()
        conf_path = default_datadir / "superaxecoin.conf"
        ...
        # (conf_path, not node.superaxecoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a superaxecoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "superaxecoin.conf" file which is ignored, because a '
        ...
        # the same superaxecoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/superaxecoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when superaxecoin.conf in the default data directory sets another datadir '
                      'and it contains a different superaxecoin.conf file that would be ignored')
        ...
        # superaxecoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "superaxecoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "superaxecoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/superaxecoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "superaxecoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "superaxecoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_superaxecoin.conf"
