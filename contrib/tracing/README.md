...
Tracing (USDT) support for SuperAxeCoin Core on Linux using. For more information on
USDT support in SuperAxeCoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `superaxecoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `superaxecoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[SuperAxeCoin Core USDT documentation] on how to list available tracepoints in your
`superaxecoind` binary.

[SuperAxeCoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/superaxecoind
```
...
SuperAxeCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/superaxecoind
```
...
In a different terminal, starting SuperAxeCoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/superaxecoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/superaxecoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/superaxecoind
```
