This directory contains the source code for the SuperAxeCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### superaxecoingui.(h/cpp)

- Represents the main window of the SuperAxeCoin UI.
...
- ClientModel is used by the main application `superaxecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a superaxecoin:... link from a browser).
...
* UI elements like SuperAxeCoinAmountField, which inherit from QWidget.
* `superaxecoinstrings.cpp`: automatically generated
* `superaxecoinunits.(h/cpp)`: AXE / mAXE / etc. handling
* `callback.h`
...
**Note:** Do not change `local/superaxecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `superaxecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `superaxecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `superaxecoin-qt`
9. You're all set! Start developing, building, and debugging the SuperAxeCoin Core GUI
